...
public class Log4JConsoleStream {
    public Log4JConsoleStream(JSONAPIInterface jsonapi) {
        jsonapi.getStreamManager().registerStream("console");
        ...
        rootLogger.addAppender(new Log4JConsoleStreamAppender(jsonapi, ThresholdFilter.createFilter("INFO", "ACCEPT", "DENY"), layout));
        ...
        Appender minecraftAppender = new Log4JConsoleStreamAppender(jsonapi, infoFilter, minecraftLayout);

        minecraftLogger.addAppender(minecraftAppender);

        ((Logger) LogManager.getLogger("com.mojang")).addAppender(minecraftAppender);

        Appender warnAppender = new Log4JConsoleStreamAppender(jsonapi, warnFilter, layout);
